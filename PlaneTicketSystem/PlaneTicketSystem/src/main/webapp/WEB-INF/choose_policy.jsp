...
<head>
    ...
    <script>
        ...
        $(function () {
            //取出结果然后进行赋值
             dep_city='<%=((Order_preparebean)request.getAttribute("order_preparebean")).getDep_city()%>';
             arr_city='<%=((Order_preparebean)request.getAttribute("order_preparebean")).getArr_city()%>';
            dep_date='<%=((Order_preparebean)request.getAttribute("order_preparebean")).getDep_date()%>';
             dep_time='<%=((Order_preparebean)request.getAttribute("order_preparebean")).getDep_time()%>';
            arr_time='<%=((Order_preparebean)request.getAttribute("order_preparebean")).getArr_time()%>';
            dep_airport='<%=((Order_preparebean)request.getAttribute("order_preparebean")).getDep_airport()%>';
            arr_airport='<%=((Order_preparebean)request.getAttribute("order_preparebean")).getArr_airport()%>';
            carrier_name='<%=((Order_preparebean)request.getAttribute("order_preparebean")).getCarrier_name()%>';
            flight_no='<%=((Order_preparebean)request.getAttribute("order_preparebean")).getFlight_no()%>';
            seat_type='<%=((Order_preparebean)request.getAttribute("order_preparebean")).getSeat_type()%>';
            seat_price='<%=((Order_preparebean)request.getAttribute("order_preparebean")).getSeat_price()%>';
            flight_id='<%=((Order_preparebean)request.getAttribute("order_preparebean")).getFlight_id()%>';


            var go_back=dep_city+"-"+arr_city
            var carrier_name_flight_no=carrier_name+" "+flight_no
            $("#dep_date").text(dep_date)
            console.log("dep_date:"+dep_date)
            $("#go_back_city").text(go_back)
            $("#seat_type").text(seat_type)
            $("#dep_time").text(dep_time)
            $("#arr_time").text(arr_time)
            $("#dep_airport").text(dep_airport)
            $("#arr_airport").text(arr_airport)
        ...
        $(function () {
            ...
            $("#passenger_type_price").text("￥"+seat_price+"/人")
            total=Number(seat_price)+50+30
            ...
            $("#passenger_type").change(function () {
                ...
                if(passenger_type=="儿童"){
                    ...
                    var child_price=Number(seat_price)*0.5
                    child_price=child_price.toString()
                    $("#passenger_type_price").text("￥"+child_price+"/人")
                    total=Number(seat_price)*0.5+50+30
                    total=total.toString()
                    $("#total_money").text("￥"+total)
                }
        ...
        function post_order(){
            ...
            if(!passenger_name){
                ...
                //然后把这些内容传到后台去
                var url="<%=basePath%>singleOrder/turn_to_pay"
                var urlfinal=url+"?"+"dep_city="+dep_city+"&arr_city="+arr_city+"&dep_date="+dep_date+"&dep_time="+dep_time+"&arr_time="+arr_time+"&dep_airport="+dep_airport+"&arr_airport="+arr_airport
                    +"&carrier_name="+carrier_name+"&flight_no="+flight_no+"&seat_type="+seat_type+"&seat_price="+seat_price+"&passenger_name="+passenger_name+"&relative_phone="+relative_phone+"&passenger_type="
                +passenger_type+"&psg_id="+psg_id+"&flight_id="+flight_id
    ...
    <div class="container">
        ...
        <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
            <ul class="nav navbar-nav navbar-right">
                <li  ><a href="../查询列表.html"><i class="icon iconfont icon-nav-block font24" >&#xe620;</i> 在线预订</a></li>
